...
prepare_coldata <- function(meta,
    ...
    for (v in c(safe_contrast, safe_covs)) {
        if (v %in% colnames(meta) && is.character(meta[[v]])) {
...
check_sample_counts <- function(coldata,
    ...
    if (is_categorical) {
        ...
        n_coeffs   <- length(covariates) + 2L   # covariates + contrast + intercept
...
main <- function(config_path, contrast_id, stratum) {
    ...
    sample_metadata <- try_logged(
        ...
        logger  = logger,
        step    = "load_metadata",
        success = sprintf("loaded %d rows", nrow(fread(cfg$inputs$sample_metadata)))
    )
    ...
    sample_metadata <- sample_metadata[sample_metadata$stratum == stratum, ]
    logger$info("filter_stratum",
        sprintf("stratum=%s | %d -> %d samples", stratum, before, nrow(sample_metadata)))
    ...
    if (!is.null(cfg$filtering$min_cells_per_sample)) {
        ...
        # Cell counts column matches stratum name
        if (stratum %in% colnames(sample_metadata)) {
            before          <- nrow(sample_metadata)
            sample_metadata <- sample_metadata[
                !is.na(sample_metadata[[stratum]]) &
                sample_metadata[[stratum]] >= min_cells, ]
            logger$info("filter_min_cells",
                sprintf("min_cells=%d | %d -> %d samples", min_cells, before, nrow(sample_metadata)))
        } else {
            logger$warn("filter_min_cells",
                sprintf("no cell count column found for stratum '%s' — skipping min_cells filter", stratum))
        }
    }
    ...
    # --- Preflight: zero-inflation check (compute early, errors added later) ---
    zi_result <- check_zero_inflation(as.matrix(raw_mat))
    if (zi_result$zero_inflated) {
        logger$error("preflight_zero_inflation",
            "100% of genes have >= 1 zero — no DESeq2 normalization method can handle this sparsity")
    } else {
    ...
    # Append zero-inflation error (computed earlier, after gene filter)
    if (zi_result$zero_inflated) {
        preflight_errors <- c(preflight_errors,
            "100% of genes have >= 1 zero — data too sparse for DESeq2 normalization")
    }
