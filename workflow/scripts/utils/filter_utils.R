...
#' Deduplicate donors: keep one biosample per donor, chosen randomly
#'
...
#' @return Deduplicated data.frame with at most one row per donor
dedup_donors <- function(meta, donor_col, paired = FALSE) {
    ...
    # In paired mode, each donor intentionally has 2 samples (one per group).
    # Deduplication would destroy the pairing — skip it entirely.
    if (paired) {
