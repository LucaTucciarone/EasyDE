...
#' Identify W factors associated with the contrast variable
#'
#' W factors that significantly correlate with the contrast variable carry
#' disease signal — including them in the model would remove real biology.
#' Identify which W factors correlate with the contrast variable and should
#' be excluded from the final model formula.
#'
...
#'   EXCLUSION DECISION: Raw p < 0.05 on the contrast_var row only.
#'     We only test one specific row so Bonferroni over all variables would
#'     be over-conservative. A lenient threshold is correct here — the cost
#'     of including a disease-correlated W (suppressing biological signal)
#'     outweighs the cost of excluding a clean W (minor loss of correction).
#'
...
#' @param contrast_var     Sanitized contrast variable name (exclusion based on this row)
#' @param biosample_id_col Column name for sample IDs
#' @return Named list: $t_mat, $p_mat_adj (Bonferroni, for heatmap),
...
identify_disease_ws <- function(coldata_with_W,
    ...
    # Run correlations — returns raw p-values
    ...
    # Bonferroni-adjusted p-values — used ONLY for the diagnostic heatmap display.
    # Shows which correlations are noteworthy across all variable × W combinations.
    p_adj_vec <- p.adjust(as.vector(corr_result$p_mat), method = "bonferroni")
    p_mat_adj <- matrix(p_adj_vec,
                        nrow     = nrow(corr_result$p_mat),
                        dimnames = dimnames(corr_result$p_mat))

    # W exclusion decision uses RAW p-values, not Bonferroni-adjusted.
    # Rationale: we only test one specific row (contrast_var), so multiple-testing
    # correction across all variables would make us over-conservative here.
    # Missing a disease-contaminated W (false negative) is worse than excluding
    # a clean W (false positive), so we use a lenient threshold.
    p_mat_raw <- corr_result$p_mat

    if (contrast_var %in% rownames(p_mat_raw)) {
        disease_ws <- colnames(p_mat_raw)[
            !is.na(p_mat_raw[contrast_var, ]) & p_mat_raw[contrast_var, ] < 0.05
        ]
    } else {
        disease_ws <- character(0)
    }

    # Safe Ws: all Ws UP TO best_k that are NOT disease-associated
    safe_ws <- latent_names[!latent_names %in% disease_ws]

    list(
        t_mat                = corr_result$t_mat,
        p_mat_adj            = p_mat_adj,
        disease_associated_Ws = disease_ws,
        safe_Ws              = safe_ws
    )
}
...
main <- function(config_path, contrast_id, stratum) {
    ...
    # Sanitize latent_corr_vars — only keep those present in coldata
    ...
    best_k <- if (max_k == 1) 1L else find_best_k(anova_df)
    ...
    logger$info("ruvseq_w_pruning",
        sprintf("disease-associated Ws (excluded from formula): %s",
