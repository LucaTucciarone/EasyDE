...
#' @param contrast_var     Sanitized contrast variable name (used for tier-1 raw-p exclusion)
#' @param biosample_id_col Column name for sample IDs
#' @return Named list: $t_mat, $p_mat_display (two-tier p-values, for heatmap),
...
identify_disease_ws <- function(coldata_with_W,
    ...
    # This matrix shows the ACTUAL p-values used for W exclusion decisions,
    # so the heatmap stars match what the pipeline does:
    #   - Contrast variable row: raw p-values (tier 1, most aggressive)
    #   - Other covariate rows: BH per-W adjusted p-values (tier 2)
    p_mat_raw <- corr_result$p_mat

    # Start with raw p-values, then overwrite non-contrast rows with BH per-W
    p_mat_display <- p_mat_raw
    other_vars <- setdiff(rownames(p_mat_raw), contrast_var)
    if (length(other_vars) > 0) {
        for (w in colnames(p_mat_raw)) {
            pvals_raw <- p_mat_raw[other_vars, w, drop = TRUE]
            valid     <- !is.na(pvals_raw)
            pvals_adj <- rep(NA_real_, length(pvals_raw))
            if (sum(valid) > 0) {
                pvals_adj[valid] <- p.adjust(pvals_raw[valid], method = "BH")
            }
            p_mat_display[other_vars, w] <- pvals_adj
        }
    }

    # --- W exclusion decision: two-tier strategy ---
    p_mat_raw <- corr_result$p_mat
    associated_ws <- character(0)

    # Tier 1: CONTRAST VARIABLE — raw p < 0.05 (most liberal, protect biology).
    # Any W that shows even a nominally significant association with the
    # contrast/disease variable is excluded. False-positive exclusion only
    # costs power; false-negative lets the W absorb real biology.
    contrast_row_name <- contrast_var
    if (contrast_row_name %in% rownames(p_mat_raw)) {
        for (w in colnames(p_mat_raw)) {
            pval <- p_mat_raw[contrast_row_name, w]
    ...
    # Tier 2: OTHER COVARIATES — BH-adjusted per-W p < 0.05.
    # For each W not already excluded, BH-correct its p-values across the
    # non-contrast variables. Exclude if ANY adjusted p < 0.05.
    other_vars <- setdiff(rownames(p_mat_raw), contrast_var)
    remaining_ws <- setdiff(colnames(p_mat_raw), associated_ws)

    if (length(other_vars) > 0 && length(remaining_ws) > 0) {
        for (w in remaining_ws) {
            ...
            if (any(!is.na(pvals_adj) & pvals_adj < 0.05)) {
                associated_ws <- c(associated_ws, w)
            }
        }
    }
    disease_ws <- associated_ws

    # Safe Ws: all Ws UP TO best_k that are NOT associated with any tested variable
