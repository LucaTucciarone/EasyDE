...
main <- function(config_path, contrast_id) {
    ...
    tryCatch({
        ...
        plot_df <- contrast_summary
        plot_df$status <- factor(plot_df$status, levels = status_levels)
        plot_df$biosample <- factor(plot_df$biosample,
                                     levels = sort(unique(plot_df$biosample)))

        # Use base DEGs as the annotation number
        plot_df$label <- ifelse(!is.na(plot_df$n_degs_base),
                                plot_df$n_degs_base, "")

        p <- ggplot(plot_df, aes(x = biosample, y = contrast, fill = status)) +
            ...
            labs(
                ...
                title = sprintf("Pipeline status: %s (numbers = base DEGs)",
                                contrast_id)
            ) +
        ...
        plot_path <- file.path(summary_dir, "status_tile.pdf")
        pdf(plot_path, width = max(6, nrow(plot_df) * 0.8), height = 4)
        print(p)
        dev.off()
        cat(sprintf("\n  Status tile plot: %s\n", plot_path))
    }, error = function(e) {
        cat(sprintf("\n  [WARN] Could not generate status tile plot: %s\n",
                    e$message))
    })
