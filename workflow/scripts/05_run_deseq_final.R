...
main <- function(config_path, contrast_id, stratum) {
    ...
    result <- if (is_categorical) {
        DESeq2::results(dds_ruv,
                        contrast = c(safe_contrast, experimental_grp, control_grp))
    } else {
        DESeq2::results(dds_ruv, name = safe_contrast)
    }
    ...
    n_degs <- sum(
        ...
        result_df$padj < cfg$deseq2$fdr_threshold &
        abs(result_df$log2FoldChange) > cfg$deseq2$l2fc_threshold,
        na.rm = TRUE
    )
    ...
    # --- LFC shrinkage: merged as second column, not separate file ---
    result_df$log2FoldChange_shrunk <- NA_real_
    shrinkage_ran <- FALSE

    if (isTRUE(cfg$deseq2$lfc_shrinkage) && is_categorical) {
        # DESeq2 uses make.names() internally to sanitize variable/level names.
        coef_name <- paste0(
            make.names(safe_contrast), "_",
            make.names(experimental_grp), "_vs_",
            make.names(control_grp)
        )
        available_coefs <- DESeq2::resultsNames(dds_ruv)
        ...
        result_shrunk <- tryCatch(
            DESeq2::lfcShrink(dds_ruv, coef = coef_name, type = "apeglm", quiet = TRUE),
            error = function(e) {
        ...
        if (!is.null(result_shrunk)) {
            ...
            shrinkage_ran <- TRUE
            logger$info("lfc_shrinkage", "log2FoldChange_shrunk added as column")
        }
    }
